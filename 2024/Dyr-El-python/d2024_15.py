...
def checkMoveDouble(grid: Grid2D, pos: Vec2D, i: str) -> bool:
    if grid[pos] == "@":
        ...
        if i in "<>":
            ...
            return checkMoveDouble(grid, DIRECTIONS[i] + pos, i) and checkMoveDouble(
                grid, DIRECTIONS[i] + pos + Vec2D(1, 0), i
            )
    elif grid[pos] in "]":
        if i in "<>":
            return checkMoveDouble(grid, DIRECTIONS[i] + pos, i)
...
def score(grid: Grid2D) -> int:
    return sum((pos.x + pos.y * 100) for pos in grid.find(lambda _, x: x in "O["))
